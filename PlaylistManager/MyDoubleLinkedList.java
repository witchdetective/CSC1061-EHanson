...
// All of this code is from class. New methods are added at the bottom.
...
public class MyDoubleLinkedList<E> implements List<E> {
	...
	// All of the new methods added for assignment.
	public int count() {
		int nodeCount = 0;
		for (Node node = head; node != null; node = node.next) {
			nodeCount++;
	...
	public MyDoubleLinkedList<E> shuffle(MyDoubleLinkedList<E> list) {
		Random randGen = new Random();

		for (int i = size - 1; i > 0; i--) {
			int newIndex = randGen.nextInt(i + 1);
			E element = list.get(newIndex);
